...
const List = ({ children }: React.PropsWithChildren) => {
  ...
  const handleAddCard = async (args: { stageId: string }) => {
    const path =
        ...
        metadata: {
          ...
          stageName:
            args.stageId === 'unassigned'
              ? 'UNASSIGNED'
              : taskStages.columns?.find((col) => col.id === args.stageId)?.title || 'UNKNOWN',
        },
      });
    } catch (error) {
      console.error('Failed to track add card action:', error);
    }
